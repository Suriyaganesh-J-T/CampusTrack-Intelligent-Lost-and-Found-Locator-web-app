    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                // tags should be a comma-separated string, e.g. "iphone,apple"
                formData.append("tags", values.tags || "");
                if (file) formData.append("image", file);

                // Important: let axios set Content-Type (boundary)
    ...
    return (
        <div className="max-w-2xl mx-auto p-4">
            ...
            <form onSubmit={formik.handleSubmit} className="bg-white p-6 rounded-lg shadow-sm">
                ...
                <input name="tags" placeholder="e.g. iphone,apple,serial123" className="w-full border rounded px-3 py-2 mb-3" onChange={formik.handleChange} value={formik.values.tags} />
                ...
                <div className="flex items-center gap-3">
                    <button type="submit" disabled={formik.isSubmitting} className="px-4 py-2 bg-sky-600 text-white rounded">Submit</button>
